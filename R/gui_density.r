...
displayTour = function (h,...)
{
	...
	if (type == "Little")
 		animate_dist(x1[VarIndex],little_tour())
	if (type == "Guided(holes)")
		animate_dist(x1[VarIndex],guided_tour(holes))
	if (type == "Guided(cm)") 
		animate_dist(x1[VarIndex],guided_tour(cm))
	if (type == "Guided(lda_pp)") 
		animate_dist(x1[VarIndex],guided_tour(lda_pp,cl=cl))
	if (type == "Local") 
 		animate_dist(x1[VarIndex],local_tour(basis_init(length(VarIndex), 2)))
}
#===============================================
