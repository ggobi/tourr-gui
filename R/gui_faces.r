...
{
	if (type == "Grand")
 		animate_faces(x1[1:length(VarIndex), 1: length(x1)],grand_tour(), center=T)
	if (type == "Little")
 		animate_faces(x1[VarIndex],little_tour())
	if (type == "Guided(holes)")
		animate_faces(x1[VarIndex],guided_tour(holes))
	if (type == "Guided(cm)") 
		animate_faces(x1[VarIndex],guided_tour(cm))
	if (type == "Guided(lda_pp)") 
		animate_faces(x1[VarIndex],guided_tour(lda_pp,cl=cl))
	if (type == "Local") 
 		animate_faces(x1[VarIndex],local_tour(basis_init(length(VarIndex), 2)))
}
#===============================================
